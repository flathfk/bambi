...
## 설정값
- **API 주소**: 옵션 화면 "서버 주소 설정"에서 변경 (기본값은 cloudflared https — 서버 재시작 시 URL 바뀌면 여기서 교체)
- **저장 엔드포인트**: 지금은 `POST /api/notes` 재사용. **영현 `/api/bookmarks` 나오면 `background.js`의 fetch URL 한 줄만 교체** (주석 표시됨)
